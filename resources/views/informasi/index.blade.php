  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header bg-primary">
        <h1 class="modal-title fs-5" id="staticBackdropLabel">Tambah Informasi</h1>
      ...
      <div class="modal-body">
        <form action="{{ url('Informasi')}}" method="post">
            ...
            @csrf 
        <div class="mb-3">
            <label for="jenis_id">Nama Jenis Bantuan</label>
            <div class="form-group">
                <select class="form-control select2 @error('jenis') is-invalid @enderror" style="width: 100%" name="jenis_id" id="jenis_id">
                    <option disabled value>Pilih Jenis Bantuan</option>
                    @foreach($jen    as $item)
                    <option value="{{$item->id}}, {{old('jenis')}}">{{$item->jenis}}</option>
                    @endforeach
                </select>
            </div>
            <label for="wargas_id">Nama Warga</label>
            <div class="form-group">
                <select class="form-control select2 @error('nama') is-invalid @enderror" style="width: 100%" name="wargas_id" id="wargas_id">
                    <option disabled value>Pilih Nama warga</option>
                    @foreach($war as $item)
                    <option value="{{$item->id}}, {{old('warga')}}">{{$item->nama}}</option>
                    @endforeach
                </select>
            </div>
            <label for="desc">Deskripsi</label>
            <input type="textarea" name="desc" id="desc" class="form-control @error('desc') is-invalid @enderror" value="{{old('desc')}}">
...
<div class="modal fade" id="staticUpdate{{ $item->id }}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form action="{{ url('informasi/'.$item->id) }}" method="post"> 
            @method('PUT')   
            @csrf 
